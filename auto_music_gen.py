...
def read_files(file):
    ...
    return notes if notes else None  # Return None for empty lists

# Retrieve paths recursively from inside the directories/files
file_path = ["schubert"]
all_files = glob.glob('/Users/abhijeet/Python/python/automatic-music-generation-codes/All Midi Files/' + file_path[0] + '/*.mid', recursive=True)
print("files")
print(all_files)

# Reading each midi file
notes_array = np.array([read_files(i) for i in tqdm(all_files, position=0, leave=True) if read_files(i) is not None], dtype=object)
print(notes_array)
